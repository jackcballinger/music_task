...
def format_artist_relations(input_df):
    artist_relation_df = pd.concat(
        [
            input_df['id'],
            pd.json_normalize(input_df['artist-relation-list'])[['type','type-id','artist.id','artist.type','artist.name']].rename(columns={'type': 'artist.role', 'type-id': 'artist.role-id'})
        ], axis=1
    )
    return (
        input_df[['id','artist_id']].dropna().drop_duplicates().reset_index(drop=True).rename(columns={'artist_id':'artist.id'}),
        artist_relation_df[['id','artist.id','artist.role-id']].dropna().drop_duplicates().reset_index(drop=True),
        artist_relation_df[['artist.role-id','artist.role']].dropna().drop_duplicates().reset_index(drop=True)
    )
...
def format_record_works(input_df):
    ...
    return record_work_relations[['id','work.id']].dropna().drop_duplicates().reset_index(drop=True)

def format_record_label(input_df):
    label_relation_df = pd.concat(
        [
            input_df['id'],
            pd.json_normalize(input_df['label-relation-list'])
        ], axis=1
    )
    return (
        label_relation_df[['id','type-id','label.id','begin','end','ended']].drop_duplicates().reset_index(drop=True),
        label_relation_df[['label.id','label.type','label.name','label.label-code']].drop_duplicates().reset_index(drop=True),
        label_relation_df[['type-id','type']].dropna().drop_duplicates().reset_index(drop=True)
    )

def format_artist_recordings(input_df):
    record_artist_mapping, record_creator_mapping, record_creator_role_mapping = format_artist_relations(input_df.explode('artist-relation-list').reset_index(drop=True))
    ...
    return {
        ...
        'RecordIsrcMapping': input_df[['id','isrc-list']].dropna().explode('isrc-list').reset_index(drop=True).rename(columns={'isrc-list':'isrc.value'}),
        'RecordWorkMapping': record_work_mapping,
        'RecordArtistMapping': record_artist_mapping,
        'RecordCreatorMapping': record_creator_mapping,
        'RecordLabelMapping': record_label_mapping
    }
