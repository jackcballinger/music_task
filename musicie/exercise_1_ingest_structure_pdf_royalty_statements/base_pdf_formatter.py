...
class BasePDFFormatter:
    def __init__(self, config):
        ...
        self._func_list = {table: [FUNC_DICT[k] for k in functions if k != 'enhance_table_data'] for table, functions in self._table_config.items()}

    def apply_table_formatting(self, input_df, table_name):
        formatted_df = input_df.copy()
        _LOGGER.info('formatting pdf tables')
        for func, args in zip(self._func_list[table_name], [v for k,v in self._table_config[table_name].items() if k != 'enhance_table_data']):
