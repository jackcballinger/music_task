...
class WcMusicCorpValidator(BasePDFValidator):
    """
    Validation Class specific to WC Music Corp pdfs
    """
    def __init__(
        self,
        config,
    ...
    def validate_amount_due(self, table_data: pd.DataFrame, table_name: str, col: str) -> dict:
        """
        Function to validate the amount due columns with the stated amount due on the front page
        """
    ...
    def validate_scope_sum(self, table_data: pd.DataFrame, table_name: str, *args) -> dict: #pylint:disable=unused-argument
        """
        Function to validate the gross payable sums as described in the scope_summary table
        """
    ...
    def validate_num_pages(self, table_data: dict) -> dict:
        """
        Function to validate that the number of pages from which information has been gathered
        is equal to the number of pages in the document
        """
        return {
    ...
    def validate_table(self, table_name: str, table_data: pd.DataFrame, table_funcs: dict) -> list:
        """
        Function to run validations against each table obtained from the pdf
        """
        return [
            func(
...
class WcMusicCorpFormatter(BasePDFFormatter):
    """
    Formatting Class specific to WC Music Corp pdfs
    """
    def format_pdf_data(self, input_page_data: list, input_page_table_numbers: dict) -> dict:
        """
    ...
    def match_correct_tables(self, input_page_data: list, input_page_table_numbers: dict) -> dict:
        """
        FUnction to match the correct tables to the correct pages and data
        """
        pdf_tables = {}
    ...
    def enchance_table_data(
        page_data_list: list, enhance_table_data_config: dict, prev_tables: dict
    ) -> list:
        """
        Function to add various pieced of information to the input tables as defined in the config
        """
...
def add_page_numbers(input_page_data: list) -> list:
    """
    Function to add page numbers to the input dataframes
    """
    for page in input_page_data:
        page["page_tables"] = [
...
def add_scope(input_page_data: list, scope_table: pd.DataFrame) -> list:
    """
    Function to add scope names to the input dataframes
    """
    for page in input_page_data:
...
def extract_track_titles(input_page_data: list) -> list:
    """
    Function to add track titles to input dataframes
    """
    for page in input_page_data:
        output_page_tables = []
