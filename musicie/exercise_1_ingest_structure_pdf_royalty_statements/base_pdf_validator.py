...
class BasePDFValidator:
    ...
    def write_validation_data(self, input_validation_data, test_definitions):
        document_result = all([x['Result']=='Pass' for x in list(chain.from_iterable(chain.from_iterable(list(input_validation_data.values()))))])
        failures = list(chain.from_iterable(chain.from_iterable([[[result for result in test_type if result['Result']=='Fail'] for test_type in v] for k, v in input_validation_data.items()]))) if not document_result else []

        template_vars = {
            "title" : "Quality Validation Report",
            "document_result": '<h4 style="color:green">Document Result: PASS<h4>' if document_result else '<h4 style="color:red">Document Result: FAIL<h4>',
